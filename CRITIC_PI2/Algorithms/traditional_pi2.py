...
# ----------------------  rollout data and store data ---------------
    ...
    def rollout_one_path(self, max_steps_per_episodes=MAX_EP_STEPS):
        ...
        while not done and t < max_steps_per_episodes:
            act = self.pi2_tradition(initial_state=obs)
...
# --------------------- different algorithms ---------
    def pi2_tradition(self, initial_state, iteration_times=5, horizon=10):
        candidate_actions = []
        for i in range(iteration_times):
            if i == 0:
                initial_action = self.sample_action(
                    initial_state.reshape([-1, s_dim]))  # initial action is supposed to be [action_dim,] narray object
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        pi2_critic.train(update_type=3)
