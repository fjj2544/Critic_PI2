'''
todo：
1. 存储逻辑存在一些问题，感觉是buffer 满了就应该存储，但是目前的写法是每次间隔一段时间就去存储。
2. 画图的基本功能没有完全被封装好，而且没有比较好的画图逻辑（需要学习对应的代码）
3. 现在没有使用折扣累计回报么？ 存储数据的时候其实没有必要加入gamma 这样会让数据脏了
4.
'''
import numpy as np
import gym
...
class PI2_Critic(object):
    def __init__(self, a_dim, s_dim, a_bound, env=None, buffer=None):
        self.num_episodes = DEFAULT_EPISODES_NUMBERS
...
# ----------------------  rollout data and store data ---------------
    ...
    def rollout_one_path(self, max_steps_per_episodes=MAX_EP_STEPS):
        ...
        while not done and t <= max_steps_per_episodes:
            ...
            t += 1
            obs = new_obs
        print(f"Rollout one path from the environment \n steps: {t} | total reward: {R} | mean reward: {R/t}")
        return path, R
    def rollout(self, episodes_numbers=DEFAULT_EPISODES_NUMBERS, max_steps_per_episodes=None):
        ...
        """
        print(f"start rollout {episodes_numbers} episodes from environment")
        count = 0
        returnsum = 0
        path_number = 0
        while (path_number < episodes_numbers):
            print(f"start rollout the {path_number} episode ")
...
# ---------------------- model training ------------
    ...
    def train(self, update_type = 1):
        self.rollout(episodes_numbers=self.num_episodes, max_steps_per_episodes=MAX_EP_STEPS)
